...
def analyze(last_class, training_start_str, grace_minutes, selected_days, selected_majors):
    ...
    # Filter major dulu
    all_students = last_class.copy()
    if selected_majors:
        all_students = all_students[all_students['major'].isin(selected_majors)]

    # Master list semua student (nama & major)
    student_info = (
        all_students[['student_id', 'student_name', 'major']]
        .drop_duplicates()
    )

    # Hari yang dianalisis
    days_to_analyze = selected_days if selected_days else ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]

    # Cross join: setiap student x setiap hari yang dipilih
    student_info = student_info.copy()
    student_info['_key'] = 1
    days_df = pd.DataFrame({'day': days_to_analyze, '_key': 1})
    full_grid = student_info.merge(days_df, on='_key').drop(columns='_key')

    # Jadwal yang ada (difilter major & hari)
    has_class = all_students[all_students['day'].isin(days_to_analyze)][
        ['student_id', 'day', 'last_end_time', 'last_course']
    ]

    # Left join: student yang ga ada kelas di hari itu -> last_end_time = NaT
    merged = full_grid.merge(has_class, on=['student_id', 'day'], how='left')

    results = []
    for _, row in merged.iterrows():
        if pd.isna(row['last_end_time']):
            # Tidak ada kelas di hari ini -> pasti bisa hadir
            results.append({
                "day": row['day'],
                "student_id": row['student_id'],
                "student_name": row['student_name'],
                "major": row['major'],
                "last_course": "— (Libur / Tidak ada kelas)",
                "last_end_time": "—",
                "last_end_dt": pd.NaT,
                "training_start": training_start_str,
                "status": "Hadir Tepat Waktu",
                "menit_setelah_mulai": 0
            })
        else:
